...
describe("CitationBadge", () => {
  ...
  it("pageEnd and chunkIndex default to null when undefined", async () => {
    const onSelectCitation = vi.fn()
    const user = userEvent.setup()
    const citationsWithoutOptionals = [
      { document: "policy.pdf", document_id: "doc1", page: 3, chunk_id: "abc" },
    ]

    render(
      <CitationProvider citations={citationsWithoutOptionals as any} retrievedContexts={testContexts} onSelectCitation={onSelectCitation}>
        <CitationBadge index={1} />
      </CitationProvider>
    )
